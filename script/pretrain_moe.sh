...
# 事前学習ジョブの実行。
prescale_grad="true"
jobname="moe_${model_size}B_tok${train_tokens_in_billion}B"
...
template_json="${megatron_deepspeed_dir}/examples_deepspeed/rebase/ds_config_moe_TEMPLATE.json"
...
deepspeed --master_port 29501 ${megatron_deepspeed_dir}/pretrain_gpt.py \
    ${megatron_options} \
    ${data_options} \
    ${deepspeed_options} \
    --wandb-group "MoE-gbs${global_batch_size}_mbs${batch_size}_log${log_interval}_zero${zero_stage}" \
